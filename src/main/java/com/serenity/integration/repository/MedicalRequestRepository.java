...
public interface MedicalRequestRepository extends JpaRepository<MedicalRequest,Long>{
    ...
    @Query(value = "select * from medicalrequest  where  practitionerid is not null and patientid is not null ORDER BY id OFFSET ?1 LIMIT 10000",nativeQuery = true)
    ...
    @Query(value = "select count(*) from medicalrequest",nativeQuery = true)
    int findByCount();
