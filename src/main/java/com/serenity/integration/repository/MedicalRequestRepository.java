...
public interface MedicalRequestRepository extends JpaRepository<MedicalRequest,Long>{
    ...
    @Query(value = "select * from medicalrequest  where visitid is  null ORDER BY id OFFSET ?1 LIMIT 1000",nativeQuery = true)
    ...
    @Query(value = "select count(*) from medicalrequest where  encounterid is   null",nativeQuery = true)
    int findByCount();
